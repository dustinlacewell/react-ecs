...
export const DOMParticles = () => {
    ...
    const { sizeRange, offset, delay, strength, gravity, lifetime } =
    ...
    const randomVector = (s: number) => new Vector2(rnd(s), rnd(s));
    const rndPos = () => {
        const container = containerRef.current;

        const containerPos = container
            ? new Vector2(container.offsetWidth / 2, container.offsetHeight / 2)
            : new Vector2(0, 0);

        return randomVector(offset).add(containerPos);
    };
        ...
        <div ref={containerRef}>
            <ECS.Provider>
                ...
                <Emitter emissionDelay={delay}>
                    {() => {
                        const size =
                            sizeRange[0] + Math.random() * sizeRange[1];
